...
{
	$(".checkbox-replace:not(.dg-cb-replacement), .radio-replace:not(.dg-cb-replacement)").each(function(i, el)
	{
		var $this = $(el),
			$input = $this.find('input'),
		...
		{	
			if(is_radio)
			{
				$(".dg-cb-replacement input[type=radio][name='"+name+"']").closest('.dg-cb-replacement').removeClass(checked_class);
			}
